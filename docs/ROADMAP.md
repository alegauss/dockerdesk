...
- 📋 **DD134** (deps: —) **A slow ping through the relay reads as a dead engine, so the host terminates a daemon that is working** — A quiet poll means unknown, not dead: only the held daemon process being gone or the distribution being unregistered brings the engine down. → §DD134
- 📋 **DD135** (deps: —) **The engine does not come up when FreeWilly does, so every session starts with the daemon stopped** — The tray starts the engine as it launches, under a setting that ships on, so turning it off restores the run-when-asked behaviour. → §DD135
