...
- 📋 **DD41** (deps: DD40) **A static host needs a file per route, and nothing checks that a route has a page or a page a title** — One render per route with the head patched by replace-or-throw, and the route map asserted against the metadata table in both directions, so a drift is a red build. → §DD41
- 📋 **DD42** (deps: DD41) **A Claude-first product answers an agent only in hydrated markup, in the first thing anybody reads** — Every route also emitted as Markdown from the same render, a manifest listing the routes and their sizes, and llms.txt as the whole product in one file an agent can grep. → §DD42
