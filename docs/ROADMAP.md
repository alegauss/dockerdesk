...
- 📋 **DD35** (deps: DD34 ✅) **Three lists, the engine, the logs and the shell share one window class, so a fourth list grows the same two files** — claude-tray's shell owns only the chrome and each destination its own header and footer; here every list repeats the header, empty state and refresh in full. → §DD35
