...
- 📋 **DD86** (deps: —) **The old name survives as adoption code and an agent skill, for a product that has no installed base to adopt** — Three shipped tasks bought compatibility with installs that were never made, and the cost is a second name in the paths, the labels, the rival probe and the installer. → §DD86
