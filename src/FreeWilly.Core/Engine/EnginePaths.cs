...
public sealed class EnginePaths
{
    ...
    /// <remarks>
    /// Still the old spelling, and deliberately: DD54 renamed the build and left this alone, because
    /// this root and <see cref="DistributionName"/> are state on a user's machine rather than text in
    /// a build. Renamed with no migration they start an empty engine beside a full one. DD55 owns the
    /// move, and until it lands these two names being the odd ones out is the honest state.
    /// </remarks>
    public EnginePaths()
        : this(Path.Combine(
