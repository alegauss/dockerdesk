...
/// (<c>FreeWilly.exe --preflight --json &gt; report.json</c>) has neither problem, which is the form
...
internal static class ParentConsole
{
    ...
    {
        // Only when the caller has not already redirected us. `FreeWilly.exe --preflight > file`
