...
///
/// <para><b>It takes four actions and knows the engine only as a state</b> (L6). Handed no-ops it
/// still builds, which is what lets <c>--show-menu</c> put one on the screen with no engine, no icon
/// and no window behind it — the same law that lets the window draw without a daemon.</para>
///
/// <para><b>Short on purpose.</b> A context menu that grows into a second UI is how a tray app stops
/// being glanceable; everything else belongs in the window.</para>
/// </remarks>
internal sealed class TrayMenu
{
    ...
    /// <param name="openWindow">What the third does.</param>
    /// <param name="quit">What the last does.</param>
    internal TrayMenu(Action startEngine, Action stopEngine, Action openWindow, Action quit)
    {
