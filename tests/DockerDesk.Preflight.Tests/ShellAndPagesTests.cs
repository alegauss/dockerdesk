...
public sealed class ShellAndPagesTests
{
    ...
    private static string Shell(string extension = ".xaml") =>
        File.ReadAllText(Path.Combine(RepositoryRoot(), "src/DockerDesk.Tray/Ui/MainWindow.xaml" + (extension == ".xaml" ? "" : ".cs")));

    private static IEnumerable<string> PageMarkup() =>
        Directory.EnumerateFiles(
            Path.Combine(RepositoryRoot(), "src/DockerDesk.Tray/Ui/Pages"), "*Page.xaml");
    ...
    public void The_shell_is_a_fraction_of_what_it_replaced()
    {
        ...
        var markup = File.ReadAllLines(
            Path.Combine(RepositoryRoot(), "src/DockerDesk.Tray/Ui/MainWindow.xaml")).Length;
        var code = File.ReadAllLines(
            Path.Combine(RepositoryRoot(), "src/DockerDesk.Tray/Ui/MainWindow.xaml.cs")).Length;
