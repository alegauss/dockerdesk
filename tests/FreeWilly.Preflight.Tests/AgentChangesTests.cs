...
public sealed class AgentChangesTests
{
    ...
    private const string Events = """
        {"Type":"container","Action":"start","Actor":{"ID":"aaaa","Attributes":{"name":"shop-worker-1","freewilly.session":"repro-17"}},"time":1}
        {"Type":"container","Action":"die","Actor":{"ID":"aaaa","Attributes":{"name":"shop-worker-1","freewilly.session":"repro-17","exitCode":"137"}},"time":2}
    ...
    public async Task Session_without_a_cursor_is_still_DD29s_listing_and_asks_no_events_at_all()
    {
        ...
        await using var daemon = new FakeDockerDaemon()
            ...
            .Json(Path("containers/json?all=1"),
                """[{"Id":"aaaa","Names":["/mine-a"],"Image":"shop/api:latest","State":"exited","Labels":{"freewilly.session":"repro-17"},"Ports":[]},"""
